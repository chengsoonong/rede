...
function zoom_arc() {
    ...
    select_snp_stat_range(1);

    histogram_degree_SNPs(file_json, 0);
    histogram_edges_subgraphId(file_json, 1);

    show_snp_pairs_list(file_name, st_chosen, 0, 1);
        ...
        function brushmove() {
            ...
            if (if_zoom) {
                ...
                d3.select("#chart")
                    .selectAll("g circle")
                    .filter(function(d, i) {
                        if (include_in_arr(link_selected_stat, i)) { // nodes_selected (s[0],s[1]) )
                            return d;
                        }
                    })
                .transition()
